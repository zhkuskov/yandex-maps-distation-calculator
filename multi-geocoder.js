...
MultiGeocoder.prototype.geocode = function (requests, options) {
	...
	//alert('MultiGeocoder.prototype.geocode');
        ...
        ymaps.geocode(request, opts)
            .then(
                function (response) {
					...
					//alert('MultiGeocoder.prototype.geocode then response');

                    geoObject && (result[index] = geoObject);
                    --size || (result.forEach(geoObjects.add, geoObjects), deferred.resolve({ geoObjects : geoObjects }));
                },
                function (err) {
					
					//alert('MultiGeocoder.prototype.geocode then error');
                    deferred.reject(err);
